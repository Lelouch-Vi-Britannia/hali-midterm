...
## Feature Importance
In subsequent experiments, I focused only on features such as helpfulness, summary length, text length, and TF-IDF-processed text, as I initially misunderstood the guidelines regarding the use of score-related features.
...
Interestingly, some words like "perfect" did not receive high importance scores, likely because they appeared less frequently in the dataset. This reflects the inherent inconsistency in human reviews that I previously mentioned—a word like "perfect" might intuitively seem highly positive, but its limited usage reduced its influence. On the other hand, more commonly used words like "great" conveyed a consistently positive sentiment.

The refined model achieved a score of 6.43 locally and 6.39 on the Kaggle submission, suggesting that it is less vulnerable to overfitting. The model appears to have effectively captured key information from the text, even without relying on score-related features.
